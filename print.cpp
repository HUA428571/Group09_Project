...
void PrintSingleFlight(FlightID* ID, int IDcount, int i)
{
	...
	outtextxy(380, 290, "开航日期：");
	outtextxy(550, 290, Flyday);
	outtextxy(380, 320, "基准票价：");
	outtextxy(550, 320, _itoa(ID[i].Price, IntChange, 10));
	outtextxy(380, 350, "舱位：");
	outtextxy(550, 350, ID[i].Class);
...
void PrintMultiFlight(FlightID* ID, int IDcount, int* SearchReasult, int SearchCount)
{
	...
	int page = 1;
	//	临时代码
	for (int i = 0; i < SearchCount; i += 15)
	{
		page = i/15+1;
		PrintMultiFlightPage(ID, IDcount, SearchReasult, SearchCount, page);
		Sleep(2000);
	}
...
void PrintMultiFlightPage(FlightID* ID, int IDcount, int* SearchReasult, int SearchCount, int page)//page表示当前显示第几页
{
	clearrectangle(380, 230, 1220, 680);//开始前把显示区域清空
	int Count = (page - 1) * 15;//count表示之前页数总计的航班数，即本页航班应该从count+1的下标开始
