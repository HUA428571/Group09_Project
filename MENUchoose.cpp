		...
		if (m.uMsg == WM_LBUTTONDOWN)//如果左键被按下
		{
			...
			for (int i = 0; i < 15; i++)//i用来计算行数
			{
				if (m.x > 380 && m.y > 230 + 30 * i && m.x < 1160 && m.y < 260 + 30 * i)//鼠标按在具体航班区域
					return i + 101;
				if (m.x > 1160 && m.y > 230 + 30 * i && m.x < 1220 && m.y < 260 + 30 * i)//改签
					return i + 121;
				if (m.x > 1220 && m.y > 230 + 30 * i && m.x < 1280 && m.y < 260 + 30 * i)//退票
					return i + 141;
			}
...
int CAdminMENU_ReMENU_MultiFlight_FlightDetail_MENUChoose()
{
	while (true)
	{
		...
		if (m.uMsg == WM_LBUTTONDOWN)//如果左键被按下
		{
			...
			if (m.x > 1190 && m.y > 165 && m.x < 1220 && m.y < 195)//鼠标按在返回区域
				return 18;
			if (m.x > 380 && m.y > 600 && m.x < 480 && m.y < 650)//鼠标按在退票区域
				return 28;
			if (m.x > 580 && m.y > 600 && m.x < 680 && m.y < 650)//鼠标按在改签区域
				return 29;
		}
	}
}
